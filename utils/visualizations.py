...
def grafico_faturamento(df: pd.DataFrame):
    ...
    df_vendas["Data_Fechamento"] = pd.to_datetime(df_vendas["Data_Fechamento"], errors="coerce")
    df_vendas = df_vendas.dropna(subset=["Data_Fechamento"])

    vendas_por_data = df_vendas.groupby("Data_Fechamento")["Valor"].sum().reset_index()
    ...
    fig = px.bar(
        ...
        labels={"Valor": "Faturamento", "Data_Fechamento": "Data"},
    )
